...
/*
  ...
  * MDA_f where float is one of b,i,l,f,d for fundamental byte, int, long, float, double.
    MDA<Float> where Float is any class.
...
public class MDA_f
/*
  # MDA - multidimensional array - for holding type float
*/
{
    private int dim;
    private SBHH indexer;
    ArrayList<float[]> data;  // A page at a time. (boxing would be wasteful).  Floatspc
    private int pagesize;
    
    public MDA_f(int dim)
    /*
      Constructs a new MDA (of element type float)
    */
    {
	this.dim=dim;
	data = new ArrayList<float[]>();  // Floatspc
	indexer = new SBHH();
	pagesize = 256;
    }
    ...
    */
	...
	int i_hb = i / pagesize;                                        // Floatspc
	int i_lb = i % pagesize;                                     // Floatspc
	while (i_hb >= data.size()) {data.add(new float[pagesize]);}      // Floatspc
	float[] page = data.get(i_hb);                               // Floatspc  
	page[i_lb] = value;                                      // Floatspc
    }
    ...
    */
    ...
    public void acc(int[] idx, float value)                          // Floatalg
    /*
     * Accumulates 'value' in the element of the MDA; i.e. adds 'value' to it.
     ! Be sure that idx.length matches the dim of the MDA.
    */
    {                                                            // Floatalg
	if (idx.length!=dim) {return;}                           // Floatalg
	int i = indexer.getidx(idx_to_sidx(idx));                // Floatalg
	int i_hb = i / pagesize;                                        // Floatalg
	int i_lb = i % pagesize;                                     // Floatalg
	while (i_hb >= data.size()) {data.add(new float[pagesize]);}     // Floatalg
	float[] page = data.get(i_hb);                               // Floatalg  
	page[i_lb] += value;                                     // Floatalg
    }                                                            // Floatalg
    ...
    public void acc(int i, float value)                          // Floatalg
    /*
      # Abbreviation for specific case of 1-dimensional MDA.
     */
    {                                                        // Floatalg
	if (dim>1) {return;}                                 // Floatalg
	int[] idx = new int[1];                              // Floatalg
	idx[0]=i;                                            // Floatalg
	acc(idx,value);                                      // Floatalg
    }                                                        // Floatalg
    public void acc(int i,int j, float value)                          // Floatalg
    /*
      # Abbreviation for specific case of 2-dimensional MDA.
     */
    {                                                        // Floatalg
	if (dim>2) {return;}                                 // Floatalg
	int[] idx = new int[2];                              // Floatalg
	idx[0]=i;idx[1]=j;                                            // Floatalg
	acc(idx,value);                                      // Floatalg
    }                                                        // Floatalg
    public void acc(int i,int j,int k, float value)                          // Floatalg
    /*
      # Abbreviation for specific case of 3-dimensional MDA.
     */
    {                                                        // Floatalg
	if (dim>3) {return;}                                 // Floatalg
	int[] idx = new int[3];                              // Floatalg
	idx[0]=i;idx[1]=j;idx[2]=k;                                            // Floatalg
	acc(idx,value);                                      // Floatalg
    }                                                        // Floatalg
    public void acc(int i,int j,int k,int l, float value)                          // Floatalg
    /*
      # Abbreviation for specific case of 4-dimensional MDA.
     */
    {                                                        // Floatalg
	if (dim>4) {return;}                                 // Floatalg
	int[] idx = new int[4];                              // Floatalg
	idx[0]=i;idx[1]=j;idx[2]=k;idx[3]=l;                 // Floatalg
	acc(idx,value);                                      // Floatalg
    }                                                        // Floatalg
