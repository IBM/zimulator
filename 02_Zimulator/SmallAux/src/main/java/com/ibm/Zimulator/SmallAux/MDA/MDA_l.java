...
/*
  ...
  * MDA_l where long is one of b,i,l,f,d for fundamental byte, int, long, float, double.
    MDA<Long> where Long is any class.
...
public class MDA_l
/*
  # MDA - multidimensional array - for holding type long
*/
{
    private int dim;
    private SBHH indexer;
    ArrayList<long[]> data;  // A page at a time. (boxing would be wasteful).  Longspc
    private int pagesize;
    
    public MDA_l(int dim)
    /*
      Constructs a new MDA (of element type long)
    */
    {
	this.dim=dim;
	data = new ArrayList<long[]>();  // Longspc
	indexer = new SBHH();
	pagesize = 256;
    }
    ...
    */
	...
	int i_hb = i / pagesize;                                        // Longspc
	int i_lb = i % pagesize;                                     // Longspc
	while (i_hb >= data.size()) {data.add(new long[pagesize]);}      // Longspc
	long[] page = data.get(i_hb);                               // Longspc  
	page[i_lb] = value;                                      // Longspc
    }
    ...
    */
    ...
    public void acc(int[] idx, long value)                          // Longalg
    /*
     * Accumulates 'value' in the element of the MDA; i.e. adds 'value' to it.
     ! Be sure that idx.length matches the dim of the MDA.
    */
    {                                                            // Longalg
	if (idx.length!=dim) {return;}                           // Longalg
	int i = indexer.getidx(idx_to_sidx(idx));                // Longalg
	int i_hb = i / pagesize;                                        // Longalg
	int i_lb = i % pagesize;                                     // Longalg
	while (i_hb >= data.size()) {data.add(new long[pagesize]);}     // Longalg
	long[] page = data.get(i_hb);                               // Longalg  
	page[i_lb] += value;                                     // Longalg
    }                                                            // Longalg
    ...
    public void acc(int i, long value)                          // Longalg
    /*
      # Abbreviation for specific case of 1-dimensional MDA.
     */
    {                                                        // Longalg
	if (dim>1) {return;}                                 // Longalg
	int[] idx = new int[1];                              // Longalg
	idx[0]=i;                                            // Longalg
	acc(idx,value);                                      // Longalg
    }                                                        // Longalg
    public void acc(int i,int j, long value)                          // Longalg
    /*
      # Abbreviation for specific case of 2-dimensional MDA.
     */
    {                                                        // Longalg
	if (dim>2) {return;}                                 // Longalg
	int[] idx = new int[2];                              // Longalg
	idx[0]=i;idx[1]=j;                                            // Longalg
	acc(idx,value);                                      // Longalg
    }                                                        // Longalg
    public void acc(int i,int j,int k, long value)                          // Longalg
    /*
      # Abbreviation for specific case of 3-dimensional MDA.
     */
    {                                                        // Longalg
	if (dim>3) {return;}                                 // Longalg
	int[] idx = new int[3];                              // Longalg
	idx[0]=i;idx[1]=j;idx[2]=k;                                            // Longalg
	acc(idx,value);                                      // Longalg
    }                                                        // Longalg
    public void acc(int i,int j,int k,int l, long value)                          // Longalg
    /*
      # Abbreviation for specific case of 4-dimensional MDA.
     */
    {                                                        // Longalg
	if (dim>4) {return;}                                 // Longalg
	int[] idx = new int[4];                              // Longalg
	idx[0]=i;idx[1]=j;idx[2]=k;idx[3]=l;                 // Longalg
	acc(idx,value);                                      // Longalg
    }                                                        // Longalg
