...
/*
  ...
  * MDA_bool where boolean is one of b,i,l,f,d for fundamental byte, int, long, float, double.
    MDA<Boolean> where Boolean is any class.
...
public class MDA_bool
/*
  # MDA - multidimensional array - for holding type boolean
*/
{
    private int dim;
    private SBHH indexer;
    ArrayList<boolean[]> data;  // A page at a time. (boxing would be wasteful).  Booleanspc
    private int pagesize;
    
    public MDA_bool(int dim)
    /*
      Constructs a new MDA (of element type boolean)
    */
    {
	this.dim=dim;
	data = new ArrayList<boolean[]>();  // Booleanspc
	indexer = new SBHH();
	pagesize = 256;
    }
    ...
    */
	...
	int i_hb = i / pagesize;                                        // Booleanspc
	int i_lb = i % pagesize;                                     // Booleanspc
	while (i_hb >= data.size()) {data.add(new boolean[pagesize]);}      // Booleanspc
	boolean[] page = data.get(i_hb);                               // Booleanspc  
	page[i_lb] = value;                                      // Booleanspc
    }
