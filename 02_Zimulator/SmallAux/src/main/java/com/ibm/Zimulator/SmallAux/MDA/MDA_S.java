...
/*
  ...
  * MDA_S where String is one of b,i,l,f,d for fundamental byte, int, long, float, double.
    MDA<String> where String is any class.
...
public class MDA_S
/*
  # MDA - multidimensional array - for holding type String
*/
{
    private int dim;
    private SBHH indexer;
    ArrayList<String[]> data;  // A page at a time. (boxing would be wasteful).  Stringspc
    private int pagesize;
    
    public MDA_S(int dim)
    /*
      Constructs a new MDA (of element type String)
    */
    {
	this.dim=dim;
	data = new ArrayList<String[]>();  // Stringspc
	indexer = new SBHH();
	pagesize = 256;
    }
    ...
    */
	...
	int i_hb = i / pagesize;                                        // Stringspc
	int i_lb = i % pagesize;                                     // Stringspc
	while (i_hb >= data.size()) {data.add(new String[pagesize]);}      // Stringspc
	String[] page = data.get(i_hb);                               // Stringspc  
	page[i_lb] = value;                                      // Stringspc
    }
    ...
    */
    ...
    public void acc(int[] idx, String value)                          // Stringalg
    /*
     * Accumulates 'value' in the element of the MDA; i.e. adds 'value' to it.
     ! Be sure that idx.length matches the dim of the MDA.
    */
    {                                                            // Stringalg
	if (idx.length!=dim) {return;}                           // Stringalg
	int i = indexer.getidx(idx_to_sidx(idx));                // Stringalg
	int i_hb = i / pagesize;                                        // Stringalg
	int i_lb = i % pagesize;                                     // Stringalg
	while (i_hb >= data.size()) {data.add(new String[pagesize]);}     // Stringalg
	String[] page = data.get(i_hb);                               // Stringalg  
	page[i_lb] += value;                                     // Stringalg
    }                                                            // Stringalg
    ...
    public void acc(int i, String value)                          // Stringalg
    /*
      # Abbreviation for specific case of 1-dimensional MDA.
     */
    {                                                        // Stringalg
	if (dim>1) {return;}                                 // Stringalg
	int[] idx = new int[1];                              // Stringalg
	idx[0]=i;                                            // Stringalg
	acc(idx,value);                                      // Stringalg
    }                                                        // Stringalg
    public void acc(int i,int j, String value)                          // Stringalg
    /*
      # Abbreviation for specific case of 2-dimensional MDA.
     */
    {                                                        // Stringalg
	if (dim>2) {return;}                                 // Stringalg
	int[] idx = new int[2];                              // Stringalg
	idx[0]=i;idx[1]=j;                                            // Stringalg
	acc(idx,value);                                      // Stringalg
    }                                                        // Stringalg
    public void acc(int i,int j,int k, String value)                          // Stringalg
    /*
      # Abbreviation for specific case of 3-dimensional MDA.
     */
    {                                                        // Stringalg
	if (dim>3) {return;}                                 // Stringalg
	int[] idx = new int[3];                              // Stringalg
	idx[0]=i;idx[1]=j;idx[2]=k;                                            // Stringalg
	acc(idx,value);                                      // Stringalg
    }                                                        // Stringalg
    public void acc(int i,int j,int k,int l, String value)                          // Stringalg
    /*
      # Abbreviation for specific case of 4-dimensional MDA.
     */
    {                                                        // Stringalg
	if (dim>4) {return;}                                 // Stringalg
	int[] idx = new int[4];                              // Stringalg
	idx[0]=i;idx[1]=j;idx[2]=k;idx[3]=l;                 // Stringalg
	acc(idx,value);                                      // Stringalg
    }                                                        // Stringalg
