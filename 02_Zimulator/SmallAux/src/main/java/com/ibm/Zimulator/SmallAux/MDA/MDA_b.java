...
/*
  ...
  * MDA_b where byte is one of b,i,l,f,d for fundamental byte, int, long, float, double.
    MDA<Byte> where Byte is any class.
...
public class MDA_b
/*
  # MDA - multidimensional array - for holding type byte
*/
{
    private int dim;
    private SBHH indexer;
    ArrayList<byte[]> data;  // A page at a time. (boxing would be wasteful).  Bytespc
    private int pagesize;
    
    public MDA_b(int dim)
    /*
      Constructs a new MDA (of element type byte)
    */
    {
	this.dim=dim;
	data = new ArrayList<byte[]>();  // Bytespc
	indexer = new SBHH();
	pagesize = 256;
    }
    ...
    */
	...
	int i_hb = i / pagesize;                                        // Bytespc
	int i_lb = i % pagesize;                                     // Bytespc
	while (i_hb >= data.size()) {data.add(new byte[pagesize]);}      // Bytespc
	byte[] page = data.get(i_hb);                               // Bytespc  
	page[i_lb] = value;                                      // Bytespc
    }
    ...
    */
    ...
    public void acc(int[] idx, byte value)                          // Bytealg
    /*
     * Accumulates 'value' in the element of the MDA; i.e. adds 'value' to it.
     ! Be sure that idx.length matches the dim of the MDA.
    */
    {                                                            // Bytealg
	if (idx.length!=dim) {return;}                           // Bytealg
	int i = indexer.getidx(idx_to_sidx(idx));                // Bytealg
	int i_hb = i / pagesize;                                        // Bytealg
	int i_lb = i % pagesize;                                     // Bytealg
	while (i_hb >= data.size()) {data.add(new byte[pagesize]);}     // Bytealg
	byte[] page = data.get(i_hb);                               // Bytealg  
	page[i_lb] += value;                                     // Bytealg
    }                                                            // Bytealg
    ...
    public void acc(int i, byte value)                          // Bytealg
    /*
      # Abbreviation for specific case of 1-dimensional MDA.
     */
    {                                                        // Bytealg
	if (dim>1) {return;}                                 // Bytealg
	int[] idx = new int[1];                              // Bytealg
	idx[0]=i;                                            // Bytealg
	acc(idx,value);                                      // Bytealg
    }                                                        // Bytealg
    public void acc(int i,int j, byte value)                          // Bytealg
    /*
      # Abbreviation for specific case of 2-dimensional MDA.
     */
    {                                                        // Bytealg
	if (dim>2) {return;}                                 // Bytealg
	int[] idx = new int[2];                              // Bytealg
	idx[0]=i;idx[1]=j;                                            // Bytealg
	acc(idx,value);                                      // Bytealg
    }                                                        // Bytealg
    public void acc(int i,int j,int k, byte value)                          // Bytealg
    /*
      # Abbreviation for specific case of 3-dimensional MDA.
     */
    {                                                        // Bytealg
	if (dim>3) {return;}                                 // Bytealg
	int[] idx = new int[3];                              // Bytealg
	idx[0]=i;idx[1]=j;idx[2]=k;                                            // Bytealg
	acc(idx,value);                                      // Bytealg
    }                                                        // Bytealg
    public void acc(int i,int j,int k,int l, byte value)                          // Bytealg
    /*
      # Abbreviation for specific case of 4-dimensional MDA.
     */
    {                                                        // Bytealg
	if (dim>4) {return;}                                 // Bytealg
	int[] idx = new int[4];                              // Bytealg
	idx[0]=i;idx[1]=j;idx[2]=k;idx[3]=l;                 // Bytealg
	acc(idx,value);                                      // Bytealg
    }                                                        // Bytealg
