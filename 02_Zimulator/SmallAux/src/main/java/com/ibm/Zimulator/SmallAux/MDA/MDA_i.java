...
/*
  ...
  * MDA_i where int is one of b,i,l,f,d for fundamental byte, int, long, float, double.
    MDA<Integer> where Integer is any class.
...
public class MDA_i
/*
  # MDA - multidimensional array - for holding type int
*/
{
    private int dim;
    private SBHH indexer;
    ArrayList<int[]> data;  // A page at a time. (boxing would be wasteful).  Integerspc
    private int pagesize;
    
    public MDA_i(int dim)
    /*
      Constructs a new MDA (of element type int)
    */
    {
	this.dim=dim;
	data = new ArrayList<int[]>();  // Integerspc
	indexer = new SBHH();
	pagesize = 256;
    }
    ...
    */
	...
	int i_hb = i / pagesize;                                        // Integerspc
	int i_lb = i % pagesize;                                     // Integerspc
	while (i_hb >= data.size()) {data.add(new int[pagesize]);}      // Integerspc
	int[] page = data.get(i_hb);                               // Integerspc  
	page[i_lb] = value;                                      // Integerspc
    }
    ...
    */
    ...
    public void acc(int[] idx, int value)                          // Integeralg
    /*
     * Accumulates 'value' in the element of the MDA; i.e. adds 'value' to it.
     ! Be sure that idx.length matches the dim of the MDA.
    */
    {                                                            // Integeralg
	if (idx.length!=dim) {return;}                           // Integeralg
	int i = indexer.getidx(idx_to_sidx(idx));                // Integeralg
	int i_hb = i / pagesize;                                        // Integeralg
	int i_lb = i % pagesize;                                     // Integeralg
	while (i_hb >= data.size()) {data.add(new int[pagesize]);}     // Integeralg
	int[] page = data.get(i_hb);                               // Integeralg  
	page[i_lb] += value;                                     // Integeralg
    }                                                            // Integeralg
    ...
    public void acc(int i, int value)                          // Integeralg
    /*
      # Abbreviation for specific case of 1-dimensional MDA.
     */
    {                                                        // Integeralg
	if (dim>1) {return;}                                 // Integeralg
	int[] idx = new int[1];                              // Integeralg
	idx[0]=i;                                            // Integeralg
	acc(idx,value);                                      // Integeralg
    }                                                        // Integeralg
    public void acc(int i,int j, int value)                          // Integeralg
    /*
      # Abbreviation for specific case of 2-dimensional MDA.
     */
    {                                                        // Integeralg
	if (dim>2) {return;}                                 // Integeralg
	int[] idx = new int[2];                              // Integeralg
	idx[0]=i;idx[1]=j;                                            // Integeralg
	acc(idx,value);                                      // Integeralg
    }                                                        // Integeralg
    public void acc(int i,int j,int k, int value)                          // Integeralg
    /*
      # Abbreviation for specific case of 3-dimensional MDA.
     */
    {                                                        // Integeralg
	if (dim>3) {return;}                                 // Integeralg
	int[] idx = new int[3];                              // Integeralg
	idx[0]=i;idx[1]=j;idx[2]=k;                                            // Integeralg
	acc(idx,value);                                      // Integeralg
    }                                                        // Integeralg
    public void acc(int i,int j,int k,int l, int value)                          // Integeralg
    /*
      # Abbreviation for specific case of 4-dimensional MDA.
     */
    {                                                        // Integeralg
	if (dim>4) {return;}                                 // Integeralg
	int[] idx = new int[4];                              // Integeralg
	idx[0]=i;idx[1]=j;idx[2]=k;idx[3]=l;                 // Integeralg
	acc(idx,value);                                      // Integeralg
    }                                                        // Integeralg
