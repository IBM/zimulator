...
/*
  ...
  * MDA_d where double is one of b,i,l,f,d for fundamental byte, int, long, float, double.
    MDA<Double> where Double is any class.
...
public class MDA_d
/*
  # MDA - multidimensional array - for holding type double
*/
{
    private int dim;
    private SBHH indexer;
    ArrayList<double[]> data;  // A page at a time. (boxing would be wasteful).  Doublespc
    private int pagesize;
    
    public MDA_d(int dim)
    /*
      Constructs a new MDA (of element type double)
    */
    {
	this.dim=dim;
	data = new ArrayList<double[]>();  // Doublespc
	indexer = new SBHH();
	pagesize = 256;
    }
    ...
    */
	...
	int i_hb = i / pagesize;                                        // Doublespc
	int i_lb = i % pagesize;                                     // Doublespc
	while (i_hb >= data.size()) {data.add(new double[pagesize]);}      // Doublespc
	double[] page = data.get(i_hb);                               // Doublespc  
	page[i_lb] = value;                                      // Doublespc
    }
    ...
    */
    {
	if (idx.length!=dim) {return(0);}
	int i = indexer.chkidx(idx_to_sidx(idx));
	if (i<0) {return(0);}
	int i_hb = i / pagesize;                                     // Doublespc
	int i_lb = i % pagesize;                                  // Doublespc
	double[] page = data.get(i_hb);                            // Doublespc
	return(page[i_lb]);                                   // Doublespc
    }
    ...
    */
    ...
    public void acc(int[] idx, double value)                          // Doublealg
    /*
     * Accumulates 'value' in the element of the MDA; i.e. adds 'value' to it.
     ! Be sure that idx.length matches the dim of the MDA.
    */
    {                                                            // Doublealg
	if (idx.length!=dim) {return;}                           // Doublealg
	int i = indexer.getidx(idx_to_sidx(idx));                // Doublealg
	int i_hb = i / pagesize;                                        // Doublealg
	int i_lb = i % pagesize;                                     // Doublealg
	while (i_hb >= data.size()) {data.add(new double[pagesize]);}     // Doublealg
	double[] page = data.get(i_hb);                               // Doublealg  
	page[i_lb] += value;                                     // Doublealg
    }                                                            // Doublealg
    ...
    public void acc(int i, double value)                          // Doublealg
    /*
      # Abbreviation for specific case of 1-dimensional MDA.
     */
    {                                                        // Doublealg
	if (dim>1) {return;}                                 // Doublealg
	int[] idx = new int[1];                              // Doublealg
	idx[0]=i;                                            // Doublealg
	acc(idx,value);                                      // Doublealg
    }                                                        // Doublealg
    public void acc(int i,int j, double value)                          // Doublealg
    /*
      # Abbreviation for specific case of 2-dimensional MDA.
     */
    {                                                        // Doublealg
	if (dim>2) {return;}                                 // Doublealg
	int[] idx = new int[2];                              // Doublealg
	idx[0]=i;idx[1]=j;                                            // Doublealg
	acc(idx,value);                                      // Doublealg
    }                                                        // Doublealg
    public void acc(int i,int j,int k, double value)                          // Doublealg
    /*
      # Abbreviation for specific case of 3-dimensional MDA.
     */
    {                                                        // Doublealg
	if (dim>3) {return;}                                 // Doublealg
	int[] idx = new int[3];                              // Doublealg
	idx[0]=i;idx[1]=j;idx[2]=k;                                            // Doublealg
	acc(idx,value);                                      // Doublealg
    }                                                        // Doublealg
    public void acc(int i,int j,int k,int l, double value)                          // Doublealg
    /*
      # Abbreviation for specific case of 4-dimensional MDA.
     */
    {                                                        // Doublealg
	if (dim>4) {return;}                                 // Doublealg
	int[] idx = new int[4];                              // Doublealg
	idx[0]=i;idx[1]=j;idx[2]=k;idx[3]=l;                 // Doublealg
	acc(idx,value);                                      // Doublealg
    }                                                        // Doublealg
